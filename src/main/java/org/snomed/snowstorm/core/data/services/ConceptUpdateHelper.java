...
public class ConceptUpdateHelper extends ComponentService {
	...
	private void updateInactivationIndicator(SnomedComponentWithInactivationIndicator newComponent,
		...
		updateMetadataRefset(membersRequired, "valueId", existingComponent, existingConceptFromParent, SnomedComponentWithInactivationIndicator::getInactivationIndicatorMembers,
	...
	private <T> void updateMetadataRefset(Map<String, Set<String>> membersRequired, String fieldName, T existingComponent, T existingConceptFromParent,
		...
		existingMembers.sort(Comparator.comparing(ReferenceSetMember::getReleasedEffectiveTime, Comparator.nullsLast(Comparator.reverseOrder())));

		for (ReferenceSetMember existingMember : existingMembers) {
			final String refsetId = existingMember.getRefsetId();
			if (membersRequired.containsKey(refsetId) && membersRequired.get(refsetId).contains(existingMember.getAdditionalField(fieldName))) {
				// Keep member
				if (!existingMember.isActive()) {
					existingMember.setActive(true);
					existingMember.markChanged();
					toPersist.add(existingMember);
				}
				membersRequired.get(refsetId).remove(existingMember.getAdditionalField(fieldName));
			} else {
				// Remove member
				if (existingMember.isActive() &&
						// If a different version of the selected member we do not want to persist it.
						toPersist.stream().map(ReferenceSetMember::getMemberId).noneMatch(id -> id.equals(existingMember.getMemberId()))) {
...
//		ensureAnyDuplicateMembersArePersisted(existingMembers, toPersist);
	...
	private void ensureAnyDuplicateMembersArePersisted(Collection<ReferenceSetMember> members, List<ReferenceSetMember> toPersist) {
		...
		for (ReferenceSetMember member : members) {
			if (!memberIds.add(member.getId()) && toPersist.stream().noneMatch(m -> m.getId().equals(member.getId()))) {
				member.markChanged();
