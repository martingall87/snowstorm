...
class BranchMergeServiceTest extends AbstractTest {
	...
	void testConflictConceptReleasedAndModifiedHistoricalAssociation() throws ServiceException {
		...
		final Concept mergedConcept = conceptService.find(conceptId, "MAIN/A");
		System.out.println("inactivationIndicatorMembers");
		for (ReferenceSetMember inactivationIndicatorMember : mergedConcept.getInactivationIndicatorMembers()) {
			System.out.println(inactivationIndicatorMember);
		}
		System.out.println("associationTargetMembers");
		for (ReferenceSetMember associationTargetMember : mergedConcept.getAssociationTargetMembers()) {
			System.out.println(associationTargetMember);
		}
		assertEquals(2021_08_01, mergedConcept.getReleasedEffectiveTime());
